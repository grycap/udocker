...
class ExecutionEngineCommon(object):
    ...
    def _remove_invalid_volumes(self):
        """remove invalid volumes"""
        for vol in list(self.opt["vol"]):
            if ":" in vol:
                (_, cont_path) = vol.split(":")
                if cont_path == "{}":
...
class FakechrootEngine(ExecutionEngineCommon):
    ...
    def _select_fakechroot_so(self):
        ...
        if conf.fakechroot_so:
            ...
            # Find /lib/libc.musl-x86_64.so.1 to see if the image is alpine
            libmusl_path = self._cont2host("/lib/libc.musl-x86_64.so.1")
            if os.path.isfile(libmusl_path) and os.access(libmusl_path, os.X_OK):
                image_list = ["libfakechroot-Alpine-x86_64.so"]
            else:
                lib = "libfakechroot"
        ...
        if "header" in kwargs:
            for header_item in kwargs["header"]:
                ...
                if (not str(header_item).startswith("Authorization: Bearer") or
                        "Signature=" not in self._files["url"]):
